      ...
      <div className="cert-view">
        ...
        <div className="cert-view-bottom">
          <p className="cert-view-title">
            { this.state.verified ? "✅" : "" } { certificate.title } by <Link to={"/users/" + certificate.by }>{ !certificate.issueserName ? certificate.by.substr(0, 16) : certificate.issueserName }</Link> { !certificate.to ? "" : "to " + certificate.to.substr(0, 16) } at {
